...
void controlTask(void *parameter) {
  ...
  for (;;) {
    ...
    Machine.update(now);
    // Chi sao chep snapshot da tinh xong. NetworkBridge khong co duong ghi
    // vao output/relay va tu gioi han tan suat sao chep.
    mayapNetworkSetSnapshot(Machine.config(), Machine.runtime());
    ...
    if (elapsedMs(now, lastStackReportAt) >= TASK_STACK_MONITOR_MS) {
      ...
      mayapSerialPrintf(false,
          "[TASK] stack ctrl=%u hmi=%u sup=%u bytes cycle=%luus max=%luus\n",
