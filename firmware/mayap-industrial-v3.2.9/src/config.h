...
constexpr char MAYAP_FIRMWARE_VERSION[] = "3.2.9";
constexpr char MAYAP_NETWORK_BRIDGE_VERSION[] = "1.0.0";
constexpr char MAYAP_HARDWARE_REVISION[] = "CTRL-S3-N8-R1";
constexpr char HMI_FIRMWARE_VERSION[] = "3.3.9";
...
// Trong mot me dang chay, cong tac AUTO chi chon che do DAO:
// AUTO=ON  -> dao theo lich tu dong.
// AUTO=OFF -> chi dao bang hai cong tac cung; nhiet va cac quat van tu dong.
// Cong tac quat tay khong duoc phep vo hieu hoa quat tuan hoan trong me.
...
// Sau khi cong tac nhiet bat lai, doi on dinh truoc khi xoa E130.
...
// AUTO OFF trong me chi chuyen DAO sang bang tay. Sau 5 phut moi phat E133;
// sau khi ACK, neu van o che do dao tay thi nhac lai sau 60 phut.
constexpr uint32_t TURN_MANUAL_ALARM_DELAY_MS = 5UL * 60UL * 1000UL;
constexpr uint32_t TURN_MANUAL_RENOTIFY_MS = 60UL * 60UL * 1000UL;
// Neu thay chu ky lam lich moi da qua han, khong khoi dong motor ngay trong
// giao dich luu EEPROM/HMI; dat TURN_PENDING va doi ngan de on dinh he thong.
constexpr uint32_t TURN_OVERDUE_START_DELAY_MS = 5000UL;
...
// Cau noi mang chi sao chep snapshot va day len MQTT; khong so huu output,
// khong tham gia WDT dieu khien va luon co uu tien thap hon HMI.
constexpr uint32_t NETWORK_TASK_PERIOD_MS = 10UL;
constexpr uint32_t NETWORK_PUBLISH_INTERVAL_MS = 2000UL;
constexpr uint32_t NETWORK_CONFIG_PUBLISH_INTERVAL_MS = 30000UL;
constexpr uint32_t NETWORK_RECONNECT_INTERVAL_MS = 5000UL;
constexpr uint32_t NETWORK_PORTAL_AFTER_MS = 120000UL;
constexpr size_t NETWORK_TASK_STACK_BYTES = 16384U;
...
constexpr uint8_t EVENT_LOG_RAM_SIZE = 128U;
// RAM ring buffer chi giu nhat ky cua lan cap nguon hien tai: ghi rat ben,
// khong hao mon EEPROM/flash, nhung se mat khi mat dien/reset. HMI hien 24
// su kien moi nhat trong cua so toi da 30 ngay cua chinh phien RAM nay.
constexpr uint8_t HMI_EVENT_DISPLAY_CAPACITY = 24U;
constexpr uint8_t HMI_FAULT_DISPLAY_CAPACITY = 12U;
constexpr uint32_t HMI_EVENT_WINDOW_SEC = 30UL * 86400UL;
...
enum AlarmBit : uint32_t {
  ...
  AlarmTurning     = 1UL << 5,
  // E133 co bit rieng de ACK DAO TAY khong vo tinh ACK cac loi he thong.
  AlarmTurnManual  = 1UL << 6,
  AlarmSystem      = 1UL << 7
};
constexpr uint32_t ALARM_KNOWN_MASK =
    AlarmSensor | AlarmTempLow | AlarmTempHigh | AlarmEmergency |
    AlarmHumidityLow | AlarmTurning | AlarmTurnManual | AlarmSystem;
...
struct MachineRuntime {
  ...
  // autoMode la trang thai cong tac AUTO vat ly. Trong me, AUTO OFF chi
  // chuyen he DAO sang tay; nhiet/quat van tu dong.
  bool autoMode = false;
  bool turnManualMode = false;
  bool turnManualAlarmActive = false;
  bool turnScheduleOverdue = false;
  uint8_t trayPosition = 0; // 0=khong biet, 1=trai, 2=phai
  uint8_t currentDay = 0;
  float heaterPower = 0.0f;
  bool heaterOn = false;
  bool heatMasterOn = false;
...
using HmiCommandResultCallback = void (*)(uint32_t commandId, bool ok,
                                          const char *message);
using HmiConfigResultCallback = void (*)(uint32_t transactionId, bool ok);
using HmiEventSnapshotCallback = void (*)(const HmiEventSnapshot &snapshot);
...
// API dung chung cho HMI vat ly va cau noi web. Cau noi web bat buoc di qua
// cung transaction/queue nay de MachineController tu kiem tra an toan.
