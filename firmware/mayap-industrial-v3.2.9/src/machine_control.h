...
// ============================================================================
// EVENT LOG RAM: RING BUFFER TINH, KHONG GHI FLASH/EEPROM DINH KY
// Ghi RAM khong gay hao mon. Du lieu chi ton tai trong lan cap nguon hien tai.
// ============================================================================
...
enum class EventCode : uint16_t {
  ...
  TurnScheduleOverdue, TurnScheduleRecalculated, TurnManualReminder,
  CommandRejected = 80,
  InputBase = 100,
  OutputBase = 200,
  FaultBase = 1000
  ...
  void snapshotRecent(uint32_t now, HmiEventSnapshot &out) const {
    ...
    for (uint8_t offset = 0U; offset < count_; ++offset) {
      const uint8_t index = static_cast<uint8_t>(
          (head_ + EVENT_LOG_RAM_SIZE - 1U - offset) % EVENT_LOG_RAM_SIZE);
      const EventEntry &e = entries_[index];
      // Phep tru uint32_t tren millis() an toan qua mot lan wrap. Cua so 30
      // ngay van nho hon chu ky wrap millis xap xi 49 ngay.
...
inline const FaultDescriptor &faultDescriptor(FaultCode code) {
  ...
  static const FaultDescriptor table[] = {
    ...
    // AUTO OFF trong me chi chuyen dao sang tay; khong khoa nhiet/quat.
    {FaultCode::TurnManualDuringBatch, FaultSeverity::Warning, 65U, AlarmTurnManual, false, false, false, false, false, false, "TURNING MANUAL"},
...
class FaultManager {
 public:
  ...
  bool acknowledged(FaultCode code) const {
    const FaultState *state = findConst(code);
    return state && state->active && state->acknowledged;
  }
  // Dua mot alarm van con condition tu trang thai ACK ve chua ACK de nhac lai.
  // Khong tao condition moi va khong thay doi tac dong an toan cua fault.
  bool renotify(FaultCode code, uint32_t now) {
    FaultState *state = find(code);
    if (!state || !state->active || !state->condition || !state->acknowledged) {
      return false;
    }
    state->acknowledged = false;
    state->lastAt = now;
    ...
    if (log_) log_->push(now, EventType::FaultRaised,
        static_cast<uint16_t>(EventCode::FaultBase) + static_cast<uint16_t>(code),
        state->detail, static_cast<uint8_t>(desc.severity) | 0x80U);
    mayapSerialPrintf(false, "[FAULT] RENOTIFY %u %s\n",
                     static_cast<unsigned>(code), desc.text);
    return true;
  }
 ...
 private:
  static constexpr uint8_t MAX_FAULTS = 24U;
...
class RtcDs3231 {
 ...
 private:
  ...
  bool readDateTime(DateTime &out) const {
    ...
    out.hour = fromBcd(data[2] & 0x3FU); // che do 24 gio
...
constexpr uint16_t CONFIG_SCHEMA = 3;
...
class MachineController {
 public:
  ...
  void update(uint32_t now) {
    ...
    updateBatchTime(now);
    updateLed(now);
    if (runtimeGate_.due(now, true)) copyRuntimeToHmi(true);
 ...
 private:
  ...
  void processHmiTransactions(uint32_t now) {
    ...
    if (hmiTakeSavedConfig(requested, transactionId)) {
      ...
      if (ok) {
        ...
        // Lich dao luon duoc tinh tu moc dao thanh cong/khởi tao lich gan nhat,
        // khong reset lai tu luc nhan LUU. Neu chu ky moi da qua han, dat
        // TURN_PENDING va khoi dong sau mot khoang tre an toan ngan.
        if (batchRunning_ && previous.turnIntervalMin != config_.turnIntervalMin) {
          recalculateTurnSchedule(now, previous.turnIntervalMin,
      ...
      mayapSerialPrintf(false, "[CFG] save=%s%s SV=%.1f HIGH=%.1f EMG=%.1f turn=%umin\n",
                       ok ? "OK" : "FAIL", saveAllowed ? "" : "(SAFETY_BLOCK)", requested.targetTemp,
  ...
  bool startBatch(uint32_t now, const char *&message) {
    ...
    turnCountBatch_ = 0;
    nextTurnAt_ = 0U;
    turnScheduleBaseAt_ = 0U;
    turnScheduleBaseValid_ = false;
    turnScheduleOverdue_ = false;
  ...
  bool stopBatch(uint32_t now, const char *&message) {
    ...
    turnScheduleBaseValid_ = false;
    turnScheduleOverdue_ = false;
    completingMoveBeforeManual_ = false;
    autoOffSince_ = 0U;
    turnManualAlarmActive_ = false;
    turnManualRenotifyAt_ = 0U;
  ...
  bool requestHmiManualTurn(uint32_t now, TurnDirection direction,
    ...
    // Trong mot me, HMI tuyet doi khong duoc dieu khien motor dao. Khi AUTO
    // OFF chi hai cong tac vat ly D17/D18 moi co quyen; backend van chan de
    // phong HMI/Serial gui nham lenh cu.
    if (batchRunning_ || resumePending_) {
      message = "ME DANG CHAY - DUNG NUT CUNG";
      return false;
    }
  ...
  void processResume(uint32_t now) {
    ...
    // AUTO chi chon che do DAO. Phuc hoi mot me cu khong duoc treo nhiet
    // chi vi cong tac dang o DAO TAY; moi truong van phai tu dong nhu truoc.
    // Me moi van bat buoc AUTO trong startBatch() de tranh khoi dong nham.
    if (!in.heaterEnable) {
    ...
    eventLog_.push(now, EventType::BatchStart,
                   ...
                   static_cast<int16_t>(std::min<uint32_t>(elapsedBeforeStartSec_ / 60UL,
                                                          INT16_MAX)),
                   1U);
    mayapSerialPrintf(false, "[BATCH] RESUME turnMode=%s elapsed=%lus\n",
                     in.autoMode ? "AUTO" : "MANUAL",
                     static_cast<unsigned long>(elapsedBeforeStartSec_));
  }
  ...
  void updateAlarms(uint32_t now) {
    ...
    // Trong me, AUTO OFF chi chuyen he DAO sang tay. Quat tuan hoan van
    // bi firmware ep ON, vi vay E131 khong con la condition van hanh.
    manualFanFaultActive_ = false;
    manualFanRestoreTimer_.reset();

    // E133: hien trang thai DAO TAY ngay, nhung chi phat alarm sau 5 phut.
    // Sau ACK, neu AUTO van OFF thi dua alarm ve chua ACK sau 60 phut.
    const bool turnManualMode = batchRunning_ && !in.autoMode;
    if (!turnManualMode) {
      autoOffSince_ = 0U;
      turnManualAlarmActive_ = false;
      turnManualRenotifyAt_ = 0U;
    } else {
      if (autoOffSince_ == 0U) autoOffSince_ = now;
      if (!turnManualAlarmActive_ &&
          elapsedMs(now, autoOffSince_) >= TURN_MANUAL_ALARM_DELAY_MS) {
        turnManualAlarmActive_ = true;
        turnManualRenotifyAt_ = 0U;
        eventLog_.push(now, EventType::System,
            static_cast<uint16_t>(EventCode::TurnManualReminder),
            static_cast<int16_t>(TURN_MANUAL_ALARM_DELAY_MS / 60000UL));
      }
    }
    ...
    // E132 duoc giu ma so de tuong thich log/cu phap HMI cu, nhung khong
    // con kich hoat: AUTO OFF trong me/phuc hoi chi doi quyen DAO sang tay.
    faults_.set(FaultCode::ResumeRequiresAuto, false, now);
    faults_.set(FaultCode::TurnManualDuringBatch,
                turnManualAlarmActive_, now,
                autoOffSince_ == 0U ? 0 : static_cast<int16_t>(
                    std::min<uint32_t>(elapsedMs(now, autoOffSince_) / 60000UL,
                                       INT16_MAX)));

    if (turnManualAlarmActive_ &&
        faults_.acknowledged(FaultCode::TurnManualDuringBatch)) {
      if (turnManualRenotifyAt_ == 0U) {
        turnManualRenotifyAt_ = now + TURN_MANUAL_RENOTIFY_MS;
      } else if (timeReached(now, turnManualRenotifyAt_)) {
        if (faults_.renotify(FaultCode::TurnManualDuringBatch, now)) {
          eventLog_.push(now, EventType::System,
              static_cast<uint16_t>(EventCode::TurnManualReminder), 60, 1U);
        }
        turnManualRenotifyAt_ = 0U;
      }
    } else {
  ...
  // ----------------------------- AUTO/MANUAL ---------------------------------
  uint32_t turnIntervalMs(uint16_t minutes) const {
    return static_cast<uint32_t>(minutes) * 60000UL;
  }

  void establishTurnSchedule(uint32_t now) {
    turnScheduleBaseAt_ = now;
    turnScheduleBaseValid_ = true;
    nextTurnAt_ = now + turnIntervalMs(config_.turnIntervalMin);
    turnScheduleOverdue_ = false;
  }

  void recalculateTurnSchedule(uint32_t now, uint16_t oldIntervalMin,
                               uint16_t newIntervalMin) {
    if (!batchRunning_) return;

    // Neu ban cu chua co base rieng, suy ra tu deadline cu. Day giu duoc
    // thoi gian da troi thay vi bat dau lai bo dem tai luc nhan LUU.
    if (!turnScheduleBaseValid_) {
      if (nextTurnAt_ != 0U) {
        turnScheduleBaseAt_ = nextTurnAt_ - turnIntervalMs(oldIntervalMin);
      } else {
        turnScheduleBaseAt_ = now;
      }
      turnScheduleBaseValid_ = true;
    ...
    const uint32_t newDeadline =
        turnScheduleBaseAt_ + turnIntervalMs(newIntervalMin);
    if (timeReached(now, newDeadline)) {
      nextTurnAt_ = now + TURN_OVERDUE_START_DELAY_MS;
      turnScheduleOverdue_ = true;
      eventLog_.push(now, EventType::Turning,
          static_cast<uint16_t>(EventCode::TurnScheduleOverdue),
          static_cast<int16_t>(newIntervalMin));
      mayapSerialPrintf(false,
          "[TURN] SCHEDULE OVERDUE old=%umin new=%umin -> pending %lums\n",
          static_cast<unsigned>(oldIntervalMin),
          static_cast<unsigned>(newIntervalMin),
          static_cast<unsigned long>(TURN_OVERDUE_START_DELAY_MS));
    } else {
      nextTurnAt_ = newDeadline;
      turnScheduleOverdue_ = false;
      const uint32_t remainingMin =
          (newDeadline - now + 59999UL) / 60000UL;
      eventLog_.push(now, EventType::Turning,
          static_cast<uint16_t>(EventCode::TurnScheduleRecalculated),
          static_cast<int16_t>(std::min<uint32_t>(remainingMin, INT16_MAX)));
      mayapSerialPrintf(false,
          "[TURN] SCHEDULE RECALC old=%umin new=%umin remain=%lumin\n",
          static_cast<unsigned>(oldIntervalMin),
          static_cast<unsigned>(newIntervalMin),
          static_cast<unsigned long>(remainingMin));
    }
  }
  ...
  void processInputModeTransition(uint32_t now) {
    ...
    manualConflictSince_ = 0U;

    if (!autoMode) {
      // Trong me, AUTO OFF chi chuyen he DAO sang bang tay. Nhiet, PID,
      // quat tuan hoan va quat hut khong duoc thay doi quyen dieu khien.
      // Neu dang co mot lan dao/homing hop le, cho no den hanh trinh dich
      // thay vi cat motor giua duong; nut tay bi khoa den khi hoan tat.
      const bool moving = turnPhase_ == TurnPhase::MovingLeft ||
                          turnPhase_ == TurnPhase::MovingRight ||
                          turnPhase_ == TurnPhase::DeadtimeLeft ||
                          turnPhase_ == TurnPhase::DeadtimeRight;
      completingMoveBeforeManual_ = batchRunning_ && moving;
      if (!completingMoveBeforeManual_) stopTurn(false);
      manualTurnRearmRequired_ = true;
      if (batchRunning_) autoOffSince_ = now;
    } else {
      // MANUAL -> AUTO: huy moi lease HMI/lenh tay, dung motor truoc, doi hai
      // nut vat ly nha het, sau do nhan dang vi tri va tim goc neu can.
      completingMoveBeforeManual_ = false;
      stopTurn(false);
      manualTurnRearmRequired_ = true;
      autoOffSince_ = 0U;
      ...
      needHome_ = batchRunning_ && trayPosition_ == TrayPosition::Unknown;

      if (batchRunning_) {
        recalculateTurnSchedule(now, config_.turnIntervalMin,
                                config_.turnIntervalMin);
      }
    }

    eventLog_.push(now, EventType::ModeChanged,
        static_cast<uint16_t>(autoMode ? EventCode::ModeAuto
                                       : EventCode::ModeManual));
    mayapSerialPrintf(false, "[MODE] TURNING=%s ENV=AUTO\n",
  ...
  void updateTurning(uint32_t now) {
    ...
    // Xac nhan den dich truoc khi xu ly chuyen AUTO/MANUAL.
    ...
    if (turnPhase_ == TurnPhase::MovingLeft ||
      ...
      const bool originStillActive =
          (turnPhase_ == TurnPhase::MovingLeft &&
           moveOrigin_ == TrayPosition::Right && in.limitRight) ||
          (turnPhase_ == TurnPhase::MovingRight &&
           moveOrigin_ == TrayPosition::Left && in.limitLeft);
      if (originStillActive &&
          elapsedMs(now, moveStartedAt_) >= TURN_LIMIT_RELEASE_TIMEOUT_MS) {
        latchTurnFault(FaultCode::TurnLimitStuck, "HANH TRINH KHONG NHA");
        return;
      }
      if (elapsedMs(now, moveStartedAt_) >=
          static_cast<uint32_t>(config_.turnMaxRunSec) * 1000UL) {
        latchTurnFault(FaultCode::TurnTimeout, "DAO QUA THOI GIAN");
        return;
      }
      // Chuyen dong tu dong chi duoc tiep tuc khi cam bien van hop le.
      if ((in.autoMode || completingMoveBeforeManual_) && !sensorUsable_) {
        completingMoveBeforeManual_ = false;
        stopTurn(false);
        return;
      }
    ...
    if (turnPhase_ == TurnPhase::DeadtimeLeft &&
        timeReached(now, deadtimeUntil_)) {
      beginPhysicalMove(now, TurnDirection::Left);
    } else if (turnPhase_ == TurnPhase::DeadtimeRight &&
               timeReached(now, deadtimeUntil_)) {
      beginPhysicalMove(now, TurnDirection::Right);
    }

    if (!in.autoMode) {
      if (completingMoveBeforeManual_) {
        const bool stillMoving = turnPhase_ == TurnPhase::MovingLeft ||
                                 turnPhase_ == TurnPhase::MovingRight ||
                                 turnPhase_ == TurnPhase::DeadtimeLeft ||
                                 turnPhase_ == TurnPhase::DeadtimeRight;
    ...
    // AUTO: hai cong tac dao tay va moi lenh HMI/Serial bi vo hieu hoa.
    if (!batchRunning_ || !config_.turningEnabled ||
        batchPhase_ == BatchPhase::Prestart) {
    ...
    if (batchPhase_ == BatchPhase::Homing || needHome_) {
      if (turnPhase_ == TurnPhase::Idle) {
        if (in.limitLeft) {
          trayPosition_ = TrayPosition::Left;
          finishHoming(now);
        } else if (in.limitRight) {
          trayPosition_ = TrayPosition::Right;
          finishHoming(now);
        } else {
          requestTurn(now,
              HOME_TO_LEFT ? TurnDirection::Left : TurnDirection::Right,
              true, false);
        }
      }
      return;
    ...
    if (!turnScheduleBaseValid_ || nextTurnAt_ == 0U) {
      establishTurnSchedule(now);
    }
  ...
  void updateManualTurning(uint32_t now, const InputState &in) {
    ...
    if (turnPhase_ == TurnPhase::Idle) {
      // Chi tinh la mot lan dao thanh cong neu bat dau tu hanh trinh doi dien
      // va di het sang hanh trinh dich. Di tu giua de tim vi tri khong tinh.
      const bool fullManualTraversal = batchRunning_ &&
          ((left && in.limitRight) || (right && in.limitLeft));
      requestTurn(now, left ? TurnDirection::Left : TurnDirection::Right,
                  false, fullManualTraversal);
  ...
  void finishHoming(uint32_t now) {
    ...
    // Homing khong duoc tinh la mot lan dao. Neu lich da qua han truoc khi
    // tim goc, giu TURN_PENDING va dao sang ben doi sau tre an toan ngan.
    if (turnScheduleOverdue_) {
      nextTurnAt_ = now + TURN_OVERDUE_START_DELAY_MS;
    } else if (!turnScheduleBaseValid_) {
      establishTurnSchedule(now);
    }
    mayapSerialPrintf(false, "[TURN] HOME OK pos=%s overdue=%u\n",
                     trayPosition_ == TrayPosition::Left ? "LEFT" : "RIGHT",
                     turnScheduleOverdue_ ? 1U : 0U);
  }
  ...
  void updateHeatingAndOutputs(uint32_t now) {
    ...
    if (batchRunning_) {
      // Trong me, cong tac AUTO chi chon che do DAO. Moi truong luon tu dong:
      // quat tuan hoan bat buoc ON du AUTO dang ON hay OFF.
      baseCirculation = true;
    } else if (in.autoMode) {
      // Ngoai me, PREHEAT/AutoTune van yeu cau AUTO de tranh bat nham.
      baseCirculation =
          (config_.allowHeatWithoutBatch && in.heaterEnable) ||
          autotune_.running();
    } else {
      // May dang dung: cho phep cong tac quat tay phuc vu bao tri.
    ...
    // Nhiet luon bat buoc quat da chay on dinh; AUTO OFF trong me khong duoc
    // bo qua lien dong nay vi quat van do firmware dieu khien tu dong.
    const bool fanAllowsHeat = fanStable;
  ...
  void updateBatchTime(uint32_t now) {
    if (batchRunning_ && batchPhase_ == BatchPhase::Prestart &&
      ...
      if (needHome_ && inputs_.state().autoMode) {
        ...
        if (!turnScheduleBaseValid_) establishTurnSchedule(now);
      }
    }
  ...
  void copyRuntimeToHmi(bool force) {
    ...
    runtime_.turnManualMode = batchRunning_ && !inputs_.state().autoMode;
    runtime_.turnManualAlarmActive = turnManualAlarmActive_;
    runtime_.turnScheduleOverdue = turnScheduleOverdue_;
    runtime_.trayPosition = static_cast<uint8_t>(trayPosition_);
    ...
    runtime_.heatMasterOn = outputs_.state().heatMaster;
    ...
    if (batchRunning_ && inputs_.state().autoMode && nextTurnAt_ &&
        !timeReached(now, nextTurnAt_)) {
      runtime_.nextTurnMinutes = static_cast<uint16_t>(
          std::min<uint32_t>(65535UL,
              (nextTurnAt_ - now + 59999UL) / 60000UL));
    } else {
      runtime_.nextTurnMinutes = 0U;
    }
    ...
    if (emergencyActive_) {
      ...
      state = in.autoMode ? "DANG AP - DAO AUTO" : "DANG AP - DAO TAY";
  ...
  const char *heatBlockReason(uint32_t now) const {
    ...
    const bool baseFan = batchRunning_ ? true
        : in.autoMode
            ? ((config_.allowHeatWithoutBatch && in.heaterEnable) ||
               autotune_.running())
            : in.circulationFan;
    ...
    const bool fanStable = fanCommand && fanOnSince_ != 0U &&
                           elapsedMs(now, fanOnSince_) >= FAN_PRESTART_MS;
    if (!fanStable) return "FAN_PRESTART";
  ...
  // Moc lich la luc hoan thanh lan dao hop le gan nhat; truoc lan dau la
  // luc bat dau/homing xong. Doi chu ky khong duoc reset moc nay.
  uint32_t turnScheduleBaseAt_ = 0U;
  bool turnScheduleBaseValid_ = false;
  bool turnScheduleOverdue_ = false;
  bool completingMoveBeforeManual_ = false;
  ...
  // E133: AUTO OFF trong me = dao tay, moi truong van tu dong.
  uint32_t autoOffSince_ = 0U;
  bool turnManualAlarmActive_ = false;
  uint32_t turnManualRenotifyAt_ = 0U;

  ConditionTimer heaterRestoreTimer_{};
  ConditionTimer manualFanRestoreTimer_{};
  bool heaterSwitchFaultActive_ = false;
  bool manualFanFaultActive_ = false;
